...
@app.command()
def pull_all(output_file: str = 'all_cn_funds_book.csv'):
    response = requests.post(url, headers=headers, data=payload)
    pages = response.json()['page'] + 1
    with ThreadPool(5) as p:
        result = list(tqdm(p.imap(pull_funds_perpage, range(1, pages))))
    
    funds = pd.concat(result, ignore_index=True)
    funds.to_csv(output_file, index=False)
    typer.echo(f"数据已保存到 {output_file}")
